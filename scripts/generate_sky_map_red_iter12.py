...
OUTPUT_FILE = BASE / "sky_map_red_iter12.txt"
...
def get_primary_and_fallback_sky_fields(
    ...
    """
    For a science field, return the ordered list of sky fields used in iter1+2.

    Example
    -------
    science = offset2_a
    -> primary sky field = offset2_b
    -> fallback sky field(s) = other *_b fields, e.g. offset3_b
    """
...
def get_candidate_skies(
    ...
    """
    Return ordered candidate skies as (cube_id, field), using iter1+2 rules:

    1. Same date code only
    2. Primary paired field first
    3. Fallback field(s) on the same sky side second
    """
...
def get_adjacent_two_skies(
    science_field: str,
    science_id: str,
    groups: dict[str, list[str]],
) -> list[tuple[str, str]]:
    """
    Choose the two skies for iter1+2.
    ...
    4. Nearest in frame number, preferring one below and one above if possible
    """
...
def build_iter12_sky_map(groups: dict[str, list[str]]) -> dict[str, list[dict]]:
    """
    Build the sky map for the combined iter1+2 red sky-selection scheme.
    """
    sky_map = {}
...
def write_sky_map(path: Path, sky_map: dict[str, list[dict]]) -> None:
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Proposed sky map for red iterations 1+2\n")
...
def main() -> None:
    ...
    sky_map = build_iter12_sky_map(groups)
