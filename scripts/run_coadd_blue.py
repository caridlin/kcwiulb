...
from kcwiulb.coadd.blue import coadd_blue_group
...
# ------------------------------------------------------------
# Examples:
# GROUP_SUFFIXES_TO_RUN = None         -> run all suffix groups found in master file
# GROUP_SUFFIXES_TO_RUN = ["a"]        -> run only all "*_a" fields together
# GROUP_SUFFIXES_TO_RUN = ["b"]        -> run only all "*_b" fields together
# GROUP_SUFFIXES_TO_RUN = ["a", "c"]   -> run groups a and c
GROUP_SUFFIXES_TO_RUN = ["a"]

# ------------------------------------------------------------
# FIELD FILTERING
# ------------------------------------------------------------
# Only include fields whose base name is in this list.
# Examples:
# OFFSETS_TO_RUN = None
# OFFSETS_TO_RUN = ["offset2"]
# OFFSETS_TO_RUN = ["offset2", "offset3"]
OFFSETS_TO_RUN = ["offset2", "offset3"]

# Remove entire fields if needed.
# Example: ["offset3_b"]
FIELDS_TO_EXCLUDE = []

# Remove specific cube IDs.
# FILES_TO_EXCLUDE = ["kb190101_00013", "kb190101_00014"]
...
def split_field_name(field: str) -> tuple[str, str | None]:
    """
    Split a field name like 'offset2_a' into:
        ('offset2', 'a')

    If there is no suffix, returns:
        (field, None)
    """
    if "_" not in field:
        return field, None
...
def cube_id_to_path(base: Path, channel: str, field: str, cube_id: str, product: str) -> Path:
    if product == "sky":
        suffix = "_icubes.wc.c.sky.fits"
    elif product == "sky2":
        suffix = "_icubes.wc.c.sky.sky2.fits"
    else:
...
def main():
    ...
    for i, group_name in enumerate(group_names, start=1):
        ...
        if RUN_COADD:
            ...
            exposure_path = output_dir / f"coadd_blue_{group_name}_{PRODUCT}_exposure.fits"
            diag_dir = output_dir
