...
OUTPUT_FILE = BASE / "sky_map_blue_iter1.txt"
...
def get_candidate_skies(
    ...
    """
    Return ordered candidate skies as (cube_id, field), using:
    - same date code only
    - primary paired field first
    - fallback field(s) on same sky side second
    """
...
def get_adjacent_two_skies(
    ...
    """
    Choose two skies for iter1.

    Priority:
    1. same date code
    2. primary paired sky field
    3. fallback sky field(s) on same sky side
...
def build_iter1_sky_map(groups: dict[str, list[str]]) -> dict[str, list[dict]]:
    sky_map = {}
...
def write_sky_map(path: Path, sky_map: dict):
    with open(path, "w", encoding="utf-8") as f:
        f.write("# Proposed sky map for blue iteration 1\n")
...
def main():
    ...
    sky_map = build_iter1_sky_map(groups)
