...
OUTPUT_FILE = BASE / f"sky_map_{CHANNEL}_iter1.txt"
...
def select_adjacent_two_skies(
    science_id: str,
    candidates: list[tuple[str, str]],
) -> list[tuple[str, str]]:
    """
    Select two sky frames.

    candidates contains:
        (sky_cube_id, sky_field)

    Priority:
    1. Same observing date.
    2. One frame below and one above when possible.
    3. Otherwise, the two nearest frames.
    """
    ...
    same_date.sort(key=lambda item: extract_frame_number(item[0]))
    ...
    if lower and upper:
        selected = [lower[-1], upper[0]]
    else:
        selected = sorted(
            same_date,
            key=lambda item: (
                abs(extract_frame_number(item[0]) - science_number),
...
def build_explicit_sky_map(
    ...
    if len(sky_ids) < 2:
        raise ValueError(
            "The [sky] group must contain at least two frames."
        )

    candidates = [(sky_id, "sky") for sky_id in sky_ids]
    ...
    for science_id in science_ids:
        entries.append(
            {
                "science": science_id,
                "skies": select_adjacent_two_skies(
                    science_id,
                    candidates,
                ),
            }
...
def split_field(field: str) -> tuple[str, str]:
    try:
        ...
        raise ValueError(
            f"Expected a paired field name such as offset2_a: {field}"
        ) from exc
...
def detect_layout(groups: dict[str, list[str]]) -> str:
    lower_names = {name.lower() for name in groups}
...
def write_sky_map(
    ...
    with path.open("w", encoding="utf-8") as f:
        f.write(
            f"# Proposed sky map for {CHANNEL} iteration 1\n"
        )

        if layout == "explicit":
            f.write("# Format:\n")
            f.write("# science | sky1 | sky2\n\n")
        else:
            f.write("# Includes field labels for sky resolution\n")
            f.write("# Format:\n")
            f.write("# science | sky1(field) | sky2(field)\n\n")
...
def main() -> None:
    ...
    for field, entries in sky_map.items():
        ...
        for entry in entries:
            ...
            print(f"  {entry['science']}: {sky_text}")
