...
from kcwiulb.coadd.blue_group import (
    load_cube,
    get_header2d,
    get_pxarea_arcsec,
    build_common_header,
...
def _normalize_mask_paths(mask_paths: list[str | Path], n_expected: int) -> list[Path]:
    ...
    for p in paths:
        if not p.exists():
            raise FileNotFoundError(f"Missing mask cube: {p}")
...
def coadd_red_group(
    ...
    """
    Red coadd that mirrors the original notebook.
    ...
    mask_paths
        ...
        *_icubes.wc.c.sky.cr.sky2.cr.fits
        The mask is always read from extension 2 of these files.

    product
        String used in output file names.
    """
    ...
    for i, (cube_path, mask_path) in enumerate(zip(input_paths, mask_paths), start=1):
        ...
        int_hdus.append(data)
        int_hdus_uncert.append(uncert)
        int_hdus_mask.append(mask)
    ...
    # notebook-style exposure bookkeeping
    ...
    for y_j in range(coadd_size_y):
        ...
        for x_k in range(coadd_size_x):
            ...
            for i, int_hdu in enumerate(int_hdus):
                ...
                xb0 = max(xb0, 0)
                yb0 = max(yb0, 0)
                xb1 = min(xb1 + 1, int_hdu.shape[2])
                yb1 = min(yb1 + 1, int_hdu.shape[1])
                ...
                if xb0 < xb1 and yb0 < yb1:
                    ...
                    overlap_tot = 0.0
                    coadd_data_ = np.zeros(int_hdu.shape[0])
                    coadd_var_ = np.zeros_like(coadd_data_)
                    coadd_exp_ = np.zeros(int_hdu.shape[0])
                    ...
                    for y_i in range(yb0, yb1):
                        for x_i in range(xb0, xb1):
                            ...
                            coadd_data_ += (
                                ...
                                * t_exp[i]
                                * int_hdu[:, y_i, x_i]
                                * (1.0 - int_hdus_mask[i][:, y_i, x_i].astype(float))
                            )
                            coadd_exp_ += (
                                overlap
                                * t_exp[i]
                    ...
                    # covariance block: mirrors notebook
                    for y_m in range(y_j, min(y_j + 4, coadd_size_y)):
                        for x_n in range(max(x_k - 10, 0), min(x_k + 11, coadd_size_x)):
                            ...
                            xb2 = max(xb2, 0)
                            yb2 = max(yb2, 0)
                            xb3 = min(xb3 + 1, int_hdu.shape[2])
                            yb3 = min(yb3 + 1, int_hdu.shape[1])
                            ...
                            if intersect_region.area > 0:
                                if xb2 < xb3 and yb2 < yb3:
                                    xb4, yb4, xb5, yb5 = (
                                        int(round(v))
                                        for v in list(intersect_region.exterior.bounds)
                                    )

                                    overlap_tot1 = 0.0
                                    cov_var_ = np.zeros_like(coadd_data_)
                                    ...
                                    if overlap_tot1 > px_thresh:
                                        ...
                                        if j == len(cov_coordinate) or cov_coordinate[j] != coord:
                                            cov_data.insert(
                                                j,
                                                cov_var_ / overlap_tot / overlap_tot1 * px_area_ratio**2,
                                            )
    ...
    cov_coordinate_arr = np.array(cov_coordinate, dtype=int)
    cov_data_arr = np.array(cov_data, dtype=float)

    for index, coo_ in enumerate(cov_coordinate_arr):
    ...
    np.save(cov_data_path, cov_data_arr)
    np.save(cov_coord_path, cov_coordinate_arr)
